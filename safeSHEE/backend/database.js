...
if (isProduction) {
  ...
  // ✅ ADDED: report_actions table for per-officer status tracking
  pool
    .query(
      `
  ...
  `,
    ...
    .catch((err) => console.error("Error creating report_actions table:", err));
  db = pool;
} else {
  ...
  db.serialize(() => {
    ...
    // ✅ ADDED: report_actions table for per-officer status tracking
    db.run(`CREATE TABLE IF NOT EXISTS report_actions (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      report_id INTEGER,
      officer_id INTEGER,
      status TEXT DEFAULT 'pending',
